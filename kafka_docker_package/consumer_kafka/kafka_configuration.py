...
class KafkaConsumerClient:
    def __init__(self, topic_name, bootstrap_servers):
        self.consumer = KafkaConsumer(
            topic_name,
            bootstrap_servers=bootstrap_servers,
            auto_offset_reset='latest',  # Changed to 'latest' to get only new messages
            enable_auto_commit=True,
            value_deserializer=lambda x: self.try_decode(x),
            group_id=None  # Added to ensure we get all messages
        )
        self.producer_data = []

    def consume_messages(self):
        try:
            while True:  # Changed to continuous loop
                messages = self.consumer.poll(timeout_ms=100)
                for message_set in messages.values():
                    for message in message_set:
                        yield message.value  # Changed to yield for continuous streaming
